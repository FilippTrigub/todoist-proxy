...
def _render_routing_rules(routes: dict[str, Any], upstreams: dict[str, str], sub_events: dict[str, list[str]], load_error: str) -> str:
    ...
    exceptions_html = (
        ...
        '<ul class="rule-list">'
        '<li><span class="tag tag-exception">⚠ all routes</span> <code>item:updated</code> that'
        ' <strong>moves a task into a different project</strong> is rewritten at the proxy into a'
        ' synthetic <code>item:added</code> for the destination project'
        ' (<code>_trigger: project_move</code>) — Todoist never fires <code>item:added</code> for a'
        ' move, so the rewritten event follows the <code>item:added</code> rules below, including the'
        ' future-due deferral.</li>'
        ...
        if isinstance(project_routes, list):
            ...
            for sub_name, rule in project_routes.items():
                ...
                sub_blocks.append(
                    ...
                    f'<li><span class="event-tag">item:added &amp; due-poll &amp; project-move</span>'
                    f'<ul><li>responsible/assignee = {resp_html}</li>{section_li}'
                    f'<li class="exception-note">⚠ creator is <strong>not</strong> checked'
                    f' for <code>item:added</code> (including project-move rewrites)</li></ul></li>'
